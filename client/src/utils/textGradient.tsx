      ...
      {parts.map((part, index) => {
        ...
        if (index % 2 === 1) {
          return (
            <span
              key={index}
              className={`bg-gradient-to-r ${gradient} bg-clip-text text-transparent font-semibold`}
            >
              {part}
            </span>
          );
...
export function processBadgeWithGradient(text: string, gradientKey?: string): string {
  const gradient = gradientKey ? BADGE_GRADIENTS[gradientKey as keyof typeof BADGE_GRADIENTS] : 'from-pink-500 to-purple-600';
  return `bg-gradient-to-r ${gradient} text-transparent bg-clip-text font-semibold`;
}